...
const VirtualKeyboard = React.memo<Props>(({ activeTheme, settings, updateSetting }) => {
  ...
  const triggerInputChange = (node: HTMLInputElement | HTMLTextAreaElement, value: string) => {
    ...
    if (activeElement.tagName === 'TEXTAREA' && nativeTextAreaValueSetter) {
  ...
  const insertCharacter = useCallback((char: string) => {
    ...
    triggerInputChange(input, newValue);
  ...
  }, [viewMode, insertCharacter, closeKeyboard]);
  ...
  if (!isTouchDevice || isPhysicalKeyboard) return null;

  return (
    <>
      <style>{`
        ${isVisible ? `:root { ${getThemeVars()} }` : ''}
        .key-button { position: relative; padding: 0; border: none; background: transparent; touch-action: none; user-select: none; -webkit-user-select: none; outline: none; cursor: pointer; }
        .key-button::before { content: ''; position: absolute; top: -3px; bottom: -3px; left: -2px; right: -2px; z-index: 0; }
        .v-key { position: relative; z-index: 10; width: 100%; height: 100%; overflow: hidden; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; font-weight: 600; border-radius: 0.75rem; border-width: 1px; border-style: solid; transition: transform 0.1s ease, filter 0.1s ease; transform: translateZ(0); }
        .key-button.v-key-pressed .v-key { transform: scale(0.92) translateZ(0) !important; filter: brightness(1.3) !important; transition: none !important; }
        .v-key-std { background-color: var(--kb-key-std-bg); border-color: var(--kb-border); color: var(--kb-text-std); box-shadow: 0 4px 0 var(--kb-shadow); }
        [data-shift-state="ONCE"] .v-key-std, [data-shift-state="LOCKED"] .v-key-std { text-transform: uppercase; }
        .v-key-special { background-color: var(--kb-key-special-bg); border-color: var(--kb-border); color: var(--kb-text-special); box-shadow: 0 4px 0 var(--kb-shadow); }
        .v-key-back { background-color: var(--kb-key-std-bg); border-color: rgba(136, 19, 55, 0.3); color: #fb7185; box-shadow: 0 4px 0 var(--kb-shadow); }
        .v-key-space { background-color: var(--kb-key-std-bg); border-color: var(--kb-border); color: var(--kb-text-std); box-shadow: 0 4px 0 var(--kb-shadow); }
        .v-key-action { background-color: var(--theme-color); border-color: transparent; color: #020617; box-shadow: 0 4px 0 rgba(0,0,0,0.2); }
        .v-key-shift { background-color: var(--kb-key-special-bg); border-color: var(--kb-border); color: var(--kb-text-special); box-shadow: 0 4px 0 var(--kb-shadow); }
        [data-shift-state="ONCE"] .v-key-shift { background-color: #334155; border-color: rgba(255,255,255,0.3); color: white; }
        [data-shift-state="ONCE"] .v-key-shift svg, [data-shift-state="LOCKED"] .v-key-shift svg { fill: white; stroke: white; }
        [data-shift-state="LOCKED"] .v-key-shift { background-color: #475569; border-color: rgba(255,255,255,0.5); color: white; box-shadow: 0 4px 0 var(--kb-shadow), 0 0 0 2px rgba(16, 185, 129, 0.5); }
        [data-shift-state="LOCKED"] .v-key-shift::after { content: ''; position: absolute; bottom: 6px; left: 50%; transform: translateX(-50%); width: 6px; height: 6px; background-color: #34d399; border-radius: 50%; box-shadow: 0 0 5px rgba(52,211,153,0.8); }
    `}</style>
      <AnimatePresence>
        {isVisible && (
          <motion.div
