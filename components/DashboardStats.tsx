...
const DashboardStats: React.FC<Props> = ({ stats, settings, activeTheme, tourStep }) => {
  return (
    <div className={`grid grid-cols-2 gap-4 sm:gap-6 relative transition-all duration-300 ${tourStep === 1 ? `z-[60] scale-105 ring-4 ${activeTheme.ring} ring-offset-8 ring-offset-slate-950 rounded-3xl` : ''}`}>
      <MinimalStatCard 
        label="PENDING" 
