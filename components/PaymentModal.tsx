        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
             <div className="space-y-1">
               ...
               <input required type="date" value={paymentDate} onChange={e => setPaymentDate(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl px-2 py-4 text-slate-100 text-center font-mono text-sm" />
             </div>
             <div className="space-y-1">
               <label className="text-[10px] font-black text-slate-500 ml-1 uppercase">Time (Optional)</label>
               <input type="time" value={paymentTime} onChange={e => setPaymentTime(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl px-2 py-4 text-slate-100 text-center font-mono text-sm" />
             </div>
          </div>

          <button className={`w-full py-5 ${activeTheme.bg} text-slate-950 rounded-2xl font-black shadow-lg hover:brightness-110 active:scale-95 transition-all`}>Save Payment</button>
        </form>
      </div>
    </div>
  );
