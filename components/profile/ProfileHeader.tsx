...
import { ArrowLeft, Trash2, MessageSquare, Loader2, FileText } from 'lucide-react';
...
const ProfileHeader: React.FC<Props> = ({ 
  onBack, onDelete, onShareSMS, onShareWA, onGeneratePDF, isSharing, activeTheme 
}) => {
  return (
    <div className="flex items-start justify-between px-6 pt-[calc(env(safe-area-inset-top)+1.5rem)] pb-4 shrink-0 z-20">
      ...
      <div className="flex items-center gap-4">
        ...
        >
           <div className="p-3 bg-slate-800 text-blue-400 rounded-xl group-hover:bg-slate-700 transition-colors shadow-lg border border-slate-700/50 relative">
              {isSharing ? <Loader2 className="w-5 h-5 animate-spin" /> : <MessageSquare className="w-5 h-5" />}
           </div>
        ...
        <button 
          ...
          className="flex flex-col items-center gap-1.5 group active:scale-95 transition-transform"
        >
           <div className="p-3 bg-slate-800 text-emerald-400 rounded-xl group-hover:bg-slate-700 transition-colors shadow-lg border border-slate-700/50 flex items-center justify-center relative">
              {isSharing ? (
